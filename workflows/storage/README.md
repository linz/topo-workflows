...
The `unarchive` workflow also writes the restore_location, archive_bucket and consumer parameters to a file for the `cron-copy-restore` workflow to use.
...
| aws_role_config_path | str   | `s3://linz-bucket-config-nonprod/config-write.elevation.json,s3://linz-bucket-config-nonprod/config-write.imagery.json,s3://linz-bucket-config-nonprod/config-write.topographic.json` | s3 URL or comma-separated list of s3 URLs allowing the workflow to write to a target(s).                                                                                                                                    |
