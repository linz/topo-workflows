...
export class ArgoRdsStack extends Stack {
  ...
  constructor(scope: App, id: string, props: ArgoRdsProps & StackProps) {
    ...
    // Need to decide on method to add secrets to EKS
    // Until then add manually kubectl create secret generic argo-postgres-config -n argo --from-literal=password=[mypassword] --from-literal=username=argodbuser
